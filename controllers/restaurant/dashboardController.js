...
const getDashboardStats = asyncHandler(async (req, res) => {
            ...
            $group: {
              ...
              netSales: {
                $sum: {
                  $subtract: [
                    "$subtotal",
                    { $ifNull: ["$refunds.remainingCharge", 0] },
                  ],
                },
              },
              totalTax: { $sum: "$tax.totalTaxAmount" },
              totalTips: {
                $sum: { $add: ["$restaurantTipCharge", "$deliveryTipCharge"] },
              },
              ...
              avgTip: {
                $avg: { $add: ["$restaurantTipCharge", "$deliveryTipCharge"] },
              },
            },
          },
        ],
  ...
  // Execute in parallel
  const [reportData, lostRevenueData] = await Promise.all([
    reportPromise,
    lostRevenuePromise,
  ]);
...
const exportDashboardReport = asyncHandler(async (req, res) => {
  ...
  const restaurantId = `restaurant_${req.restaurantId}`;
  const Order = getOrderModel(req.restaurantDb);
  const Restaurant = require("../../models/index").getRestaurantModel(
    req.restaurantDb,
  );
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename=report_${restaurantId}_${new Date().getTime()}.pdf`,
  );
...
/**
 * Get Day by Day Report
 * GET /api/v1/restaurant/dashboard/report/daily?startDate=YYYY-MM-DD&endDate=YYYY-MM-DD
 */
const getDayByDayReport = asyncHandler(async (req, res) => {
  const { startDate, endDate } = req.query;
  const restaurantId = `restaurant_${req.restaurantId}`;
  const Order = getOrderModel(req.restaurantDb);
  ...
  const report = await Order.aggregate([
    ...
    },
    {
      $group: {
        ...
        tax: { $sum: "$tax.totalTaxAmount" },
        discount: { $sum: "$discount.totalDiscountAmount" },
        orderCount: { $sum: 1 },
      },
    },
    { $sort: { _id: 1 } },
