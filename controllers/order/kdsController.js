...
const { getOrderModel, getRestaurantModel } = require("../../models/index");
...
const updateOrderItemStatus = asyncHandler(async (req, res) => {
  ...
  const restaurantConfig = await Restaurant.findOne({ restaurantId: req.restaurantId });
  if (restaurantConfig && restaurantConfig.kdsConfiguration && restaurantConfig.kdsConfiguration.workflow && restaurantConfig.kdsConfiguration.workflow.length > 0) {
    workflow = restaurantConfig.kdsConfiguration.workflow;
  }

  if (!workflow.includes(status)) {
    return res.status(400).json({ status: "error", message: `Invalid status. Allowed: ${workflow.join(", ")}` });
  }

  // Find item and update
  const itemIndex = order.orderItems.findIndex(i => i._id && i._id.toString() === itemId);
  ...
  if (itemIndex === -1) {
    ...
    return res.status(404).json({ status: "error", message: "Item not found in order" });
  }
...
const getKDSConfig = asyncHandler(async (req, res) => {
  ...
  const workflow = restaurant?.kdsConfiguration?.workflow || ["new", "start", "prepared", "ready"];
